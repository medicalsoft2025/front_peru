...
try {
    $Controller = new PuntuacionEntrevistaController($conn3);
    $name = "Puntuacion de entrevista";
    ...
    if ($action == "crear") {
        ...
        if ($Crear["status"]) {
            responseSuccess("$name creada correctamente");
        } else {
            responseError("$name no pudo ser creada", $Crear["message"]);
        }
        ...
        if ($Actualizar["status"]) {
            responseSuccess("$name actualizada correctamente");
        } else {
            responseError("$name no pudo ser actualizada", $Actualizar["message"]);
        }
    } else if ($action == "eliminar") {
        $id = $_POST['id'];
        $Eliminar = $Controller->eliminar($id);
        if ($Eliminar["status"]) {
            responseSuccess("$name eliminada correctamente");
        } else {
            responseError("$name no pudo ser eliminada", $Eliminar["message"]);
        }
    } else {
