...
try {
    $Controller = new IncapacidadesController($conn3);
    $name = "Incapacidad";
    ...
    if ($action == "crear") {
        ...
        if ($Crear["status"]) {
            $idIncapacidad = $Controller->obtenerMax();
            if ($_FILES["archivo"]["name"] != "") {
                
                $nombreArchivo =  date("YmdHis") . "_" .$_FILES["archivo"]["name"];
                $nombreTemporal = $_FILES["archivo"]["tmp_name"];
                $directorio = "../../ComprobantesIncapacidades/";
                if ( !is_dir($directorio) ) {
                    mkdir($directorio, 0777, true);
                }

                $directorioFinal = $directorio . $nombreArchivo;
                if (move_uploaded_file($nombreTemporal, $directorioFinal)) {
                    $Actualizar = $Controller->editar($idIncapacidad, ["archivo" => $nombreArchivo]);
                    responseSuccess("$name creado correctamente");
                }else{
                    responseError("Ocurrio un error al cargar el archivo", "");
