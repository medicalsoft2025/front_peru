...
<script>
  ...
  Object.keys(metodosPago).forEach(key => {
    optionsMetodosPago += `<option value="${key}">${metodosPago[key]}</option>`;
  });
</script>
...
<script>
  ...
  function guardarFacturaCliente() {
    let idS = ["tipoComprobante", "centroCosto", "vendedorId", "clienteId", "total-neto", "numeroAutorizacion", "total-pagado"];
    let next = true;
    let data = {};
    for (let element of idS) {
      ...
      console.log("Atributo required => " + elementoInput.attr("required"));
    ...
    if (data.clienteId == "" || data.clienteId == "0") {
      let camposCiente = ["tipoIdentificacion", "identificacion", "tipoPersona", "primerNombre", "segundoNombre", "primerApellido", "segundoApellido", "direccion", "numeroTelefono", "emailFe"];
