...
class DescuentosModel
{
    ...
    public function obtenerPorCondicion($condition, $limit = "")
    {
        $query = "SELECT * FROM {$this->table} WHERE 1=1 {$condition} AND activo = '1' {$limit}";
        $result = mysqli_query($this->conn, $query);
        $rows = [];
        if ($result) {
            $rows = mysqli_fetch_all($result, MYSQLI_ASSOC);
    ...
    {
        $query = "SELECT MAX(id) as id FROM {$this->table} WHERE activo = '1' LIMIT 1";
        $result = mysqli_query($this->conn, $query);
        if ($result) {
