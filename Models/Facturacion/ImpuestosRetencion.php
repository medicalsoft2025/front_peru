...
class ImpuestoRetencionModel
{
    ...
    public function __construct($conn)
    {
        ...
        $this->table = 'FE_ImpuestosRetenciones';
    }
    ...
    {
        $query = "SELECT * FROM {$this->table} WHERE activo = 1 {$condicion}";
        $result = mysqli_query($this->conn, $query);
        if ($result) {
            $rows = mysqli_fetch_all($result, MYSQLI_ASSOC);
        } else {
    ...
    public function obtenerPorCondicion($condition) {
        $query = "SELECT * FROM {$this->table} WHERE 1=1 {$condition} AND activo = '1' LIMIT 1";
        $result = mysqli_query($this->conn, $query);
        $rows = [];
        if ($result) {
            $rows = mysqli_fetch_all($result, MYSQLI_ASSOC);
    ...
    public function obtenerPersonalizado($columnas, $condicion) {
        $query = "SELECT {$columnas} FROM {$this->table} WHERE 1=1 AND activo = '1' {$condicion} LIMIT 1";
        $result = mysqli_query($this->conn, $query);
        if ($result) {
