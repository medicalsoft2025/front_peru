...
class EntrevistaModel
{
    ...
    public function obtenerTrabajadorPorIdEntrevista($id) {
        $query = "SELECT * FROM {$this->table} pt INNER JOIN NOM_Trabajadores tr ON tr.id = pt.idTrabajador WHERE pt.id = {$id} LIMIT 1";
        $result = mysqli_query($this->conn, $query);
        if ($result) {
            $rows = mysqli_fetch_array($result, MYSQLI_ASSOC);
        } else {
