...
var text_areas = document.getElementsByTagName('textarea');
  ...
  for (i = 0; i < text_areas.length; i++) {
    text_areas[i].setAttribute('id','ReconocimientoVoz'+i);

    var boton = document.createElement("button");
    boton.setAttribute('id','boton'+i);
    boton.setAttribute('type','button');
    boton.setAttribute('class','reconocimiento_de_voz');
    boton.setAttribute('style','float: right;position: relative;top:-50px;padding: 5px;right: 7px;');
  ...
  function reset(valor) {
    recognizing[valor] = false;
    document.getElementById("boton"+valor).innerHTML = "<i title='Iniciar Grabación' style='font-size: 18px;' class='fa fa-fw fa-microphone'>";
  }
  ...
  function toggleStartStop(valor) {
    if (recognizing[valor]) {
      ...
      document.getElementById("boton"+valor).innerHTML = "<i title='Detener Grabación' style='font-size: 18px;' class='fa fa-pause'>";
    }
    ...
    recognition[valor].onresult = function (event) {
      for (var i = event.resultIndex; i < event.results.length; ++i) {
        ...
        if (event.results[i].isFinal) {
          ...
          document.getElementById("ReconocimientoVoz"+valor).value += event.results[i][0].transcript;
