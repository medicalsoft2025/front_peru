...
import { patientService, userAvailabilityService, userService } from '../../../services/api';
import { stringToDate } from '../../../services/utilidades';
import { useProductsByType } from '../../products/hooks/useProductsByType';
import { useAppointmentBulkCreate } from './useAppointmentBulkCreate';
import { useValidateBulkAppointments } from './useValidateBulkAppointments';
...
export const useAppointmentFormLogic = ({ isOpen, onClose }) => {
    ...
    // Watchers
    const userSpecialty = useWatch({ control, name: 'user_specialty' });
    const showExamRecipeField = useWatch({ control, name: 'show_exam_recipe_field' });
    const appointmentDate = useWatch({ control, name: 'appointment_date' });
    const appointmentTime = useWatch({ control, name: 'appointment_time' });
    const appointmentType = useWatch({ control, name: 'appointment_type' });
    const patient = useWatch({ control, name: 'patient' });
    const assignedUserAvailability = useWatch({ control, name: 'assigned_user_availability' });
    const examRecipeId = useWatch({ control, name: 'exam_recipe_id' });
    ...
    useEffect(() => {
        if (assignedUserAvailability && appointmentDate) {
            ...
            const userBlocks = availableBlocks.find(
                block => block.availability_id === assignedUserAvailability.id
            )?.days.find(day => day.date === dateString)?.blocks || [];
            ...
            userBlocks.forEach(block => {
                const slots = computeTimeSlots(
                    block.start_time,
                    block.end_time,
                    assignedUserAvailability.appointment_duration
                );
    ...
    useEffect(() => {
        if (assignedUserAvailability) {
            ...
            const assistants = assignedUserAvailability.user.assistants
                .filter(assistant => currentAvailableUserIds.includes(assistant.id))
                .map(assistant => ({
                    id: assistant.id,
            ...
            for (let i = 0; i < (appointmentRepetitions || 1); i++) {
                if (currentDate) {
                    ...
                    switch (appointmentFrequency) {
                        case 'diary': currentDate.setDate(currentDate.getDate() + 1); break;
