...
export function CategoryHeaderManualSection({ category, showToast }) {
        ...
        <div className="category-header">
            <div className="category-info">
                ...
                <div className="flex-grow-1">
                    ...
                    <small className="text-muted">
                        {category.videos.length} video{category.videos.length !== 1 ? 's' : ''} disponible{category.videos.length !== 1 ? 's' : ''}
                    </small>
                </div>
            </div>
            <div className="d-flex align-items-center gap-3">
                ...
                <div className="action-buttons">
                    <Tooltip target={`.edit-cat-${category.id}`} content="Editar categoría" />
                    <Button
                        className={`p-button-rounded p-button-text p-button-sm edit-cat-${category.id}`}
                    ><i className="fas fa-edit"></i></Button>
                    <Tooltip target={`.delete-cat-${category.id}`} content="Eliminar categoría" />
                    <Button
