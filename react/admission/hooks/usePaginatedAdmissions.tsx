...
import { AdmissionTableItem } from "./useAdmissions";

export const usePaginatedAdmissions = (props: { filters: any }) => {
    ...
    const mapAdmissionToTableItem = (item: any): AdmissionTableItem => {
        return {
            ...
            admittedBy: `${item.user.first_name || ""} ${item.user.middle_name || ""
                } ${item.user.last_name || ""} ${item.user.second_last_name || ""}`,
            patientName: `${item.patient.first_name || ""} ${item.patient.middle_name || ""
    ...
    const {
        ...
        queryFn: async (paginationParams) => {
            try {
                ...
                return { mappedAdmissions, total: data.data.total };
            } catch (error) {
                console.error(error);
                return { mappedAdmissions: [], total: 0 };
            }
        }
    });
