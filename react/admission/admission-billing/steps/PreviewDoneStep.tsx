...
const PreviewDoneStep: React.FC<PreviewAndDoneStepProps> = ({
  ...
  const paymentMethodTemplate = (rowData: any) => {
    const methodLabel =
      paymentMethodOptions.find((m) => m.value === rowData.method)?.label ||
      rowData.method;
  ...
  const priceBodyTemplate = (rowData: any) => {
    return (
      <span className="fw-bold">{formatCurrency(rowData.currentPrice)}</span>
    );
  };
  ...
  const subtotalBodyTemplate = (rowData: any) => {
    ...
    const subtotalConDescuento =
      (rowData.currentPrice * rowData.quantity - (rowData.discount ?? 0)) *
      (1 + rowData.tax / 100);
    ...
    return (
      <div className="d-flex flex-column align-items-end">
        ...
        <span className={`fw-bold ${tieneDescuento ? "text-success" : ""}`}>
  ...
  const discountBodyTemplate = (rowData: any) => {
    if (!rowData.discount || rowData.discount === 0) {
      return <Tag value="0%" severity="secondary" className="p-tag-rounded" />;
    }
    ...
    return (
      ...
      >
        ...
        <h2 className="mb-3 fw-bold">
          ¡Factura Admision Generada Exitosamente!
        </h2>
        <p className="text-muted mb-4">
          La factura ha sido creada y guardada en el sistema.
        </p>
            ...
            <Button
              ...
              icon={
                isSubmitting ? (
                  ...
                  <i className="ffas fa-check me-1"></i>
                )
              }
