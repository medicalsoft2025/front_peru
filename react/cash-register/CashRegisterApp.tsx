...
export const CashRegisterApp = () => {
  ...
  const { storeInvoiceSale, toastRef: storeInvoiceToastRef } =
    useStoreInvoiceSale();
  ...
  const cashRegisterPaymentFormRef =
    React.useRef<CashRegisterPaymentFormRef>(null);
  ...
  useEffect(() => {
    const mappedData = medications.map((product) => {
      const name =
        `${product.name} ${product.concentration ? `- (${product.concentration})` : ""}`.trim();
      return {
        id: product.id,
        name,
  ...
  const handleDiscountChange = (
    ...
    const discountCalculated =
      discountType === "percentage"
        ? (subtotalProducto * discountAmount) / 100
        : discountAmount;
        ...
        <div className="d-flex gap-3">
          ...
          <div className="d-flex" style={{ width: "400px", minWidth: "400px" }}>
            ...
            >
                    ...
                    <>
                      <div className="mb-2 card shadow-sm p-2" key={product.id}>
                        <CartProductCard
                          product={product}
                          removeFromCart={() =>
                            removeFromCart(products.indexOf(product))
                          }
              ...
              <Divider />
              <div className="d-flex justify-content-between align-items-center">
                <span className="fs-8">Subtotal:</span>
                <span className="fs-8">{formatPrice(subtotal)}</span>
              </div>
              <Divider />
              <div className="d-flex justify-content-between align-items-center mb-3">
                <span className="fw-bold fs-8">Total:</span>
