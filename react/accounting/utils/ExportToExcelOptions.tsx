...
interface ExportToExcelOptions<T extends DataTableValueArray> {
    data: T;
    fileName: string;
    columns?: {
        field: string;
    ...
    dt?: React.RefObject<DataTable<T>>;
    selectionOnly?: boolean;
    excludeColumns?: string[];
}

interface ExportToPDFOptions<T extends DataTableValueArray> {
    columns: {
        field: string;
        header: string;
    }[];
...
export const exportToExcel = async <T extends DataTableValueArray>(
    ...
    try {
        const { data, fileName, columns, dt, selectionOnly, excludeColumns } = options;
...
async function loadPDFLibraries(): Promise<void> {
    try {
        ...
        // Cargar jsPDF
        const jsPDFLoaded = await loadScriptWithFallback(
            'https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js',
            'https://cdn.jsdelivr.net/npm/jspdf@2.5.1/dist/jspdf.umd.min.js'
        );

        // Cargar autoTable
        const autoTableLoaded = await loadScriptWithFallback(
...
async function loadScriptWithFallback(...urls: string[]): Promise<boolean> {
...
export const exportToPDF = async <T extends DataTableValueArray>(
    options: ExportToPDFOptions<T>
): Promise<void> => {
    try {
        ...
        doc.text('Detalle de Factura', 105, 20, { align: 'center' });
