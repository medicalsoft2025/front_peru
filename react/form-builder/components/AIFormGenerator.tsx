...
import { showNotification } from '../../../services/utilidades';

interface AIFormGeneratorProps {
    visible: boolean;
    onHide: () => void;
    onImport: (generatedConfig: DynamicFormElementConfig, targetNodeKey: string) => void;
    treeOptions: TreeNode[];
}

export const AIFormGenerator: React.FC<AIFormGeneratorProps> = ({ visible, onHide, onImport, treeOptions }) => {
    const [prompt, setPrompt] = useState('');
    const [loading, setLoading] = useState(false);
    const [generatedConfig, setGeneratedConfig] = useState<DynamicFormElementConfig | null>(null);
    ...
    const handleGenerate = async () => {
        ...
        setGeneratedConfig(null);
        try {
            const response = await aiService.generateDynamicForm({ prompt });
            if (response.data && response.data.form_structure && response.data.form_structure.length > 0) {
                ...
                showNotification({ type: 'warning', message: 'La IA no devolvió una estructura válida.' });
            }
        } catch (error) {
            console.error(error);
            showNotification({ type: 'error', message: 'Error al generar el formulario.' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const footer = (
        <div className="d-flex justify-content-end gap-2">
            <Button label="Cancelar" severity="secondary" icon={<i className="fa fa-times me-1"></i>} onClick={onHide} />
            <Button
                label="Importar"
    ...
    return (
        ...
        >
            <div className="grid">
                <div className="col-12 md:col-4 d-flex flex-column gap-3">
                    <div className="d-flex flex-column gap-2">
                        <label htmlFor="prompt" className="font-bold">Describe el formulario que deseas crear</label>
                        <InputTextarea
                            id="prompt"
                            rows={5}
                            value={prompt}
                            onChange={(e) => setPrompt(e.target.value)}
                            placeholder="Ej: Historia clínica de endocrinología con datos del paciente, antecedentes familiares, medicamentos actuales..."
                            className="w-100"
                        />
                        <Button
                    ...
                    {generatedConfig && (
                        <div className="d-flex flex-column gap-2 mt-3">
                            <label htmlFor="targetNode" className="font-bold">Seleccionar ubicación de importación</label>
                            <TreeSelect
                                value={targetNodeKey}
                                options={treeOptions}
                                onChange={(e) => setTargetNodeKey(e.value as string)}
                                placeholder="Seleccionar nodo destino"
                                className="w-100"
                                filter
                                filterMode="strict"
                            />
                            <small className="text-muted">El formulario generado se agregará como hijo del nodo seleccionado.</small>
                        </div>
                    )}
                </div>

                <div className="col-12 md:col-8">
                    <label className="font-bold mb-2 d-block">Previsualización</label>
                    <div className="border rounded p-3 bg-light" style={{ minHeight: '400px', maxHeight: '600px', overflowY: 'auto' }}>
                        {loading ? (
                            <div className="d-flex justify-content-center align-items-center h-100 text-muted">
                                <i className="fa fa-spinner fa-spin me-1"></i>
                            </div>
                        ) : generatedConfig ? (
                            <DynamicForm config={generatedConfig} onSubmit={() => { }} />
                        ) : (
                            <div className="d-flex justify-content-center align-items-center h-100 text-muted">
                                Genera un formulario para ver la previsualización aquí.
                            </div>
                        )}
                    </div>
                </div>
