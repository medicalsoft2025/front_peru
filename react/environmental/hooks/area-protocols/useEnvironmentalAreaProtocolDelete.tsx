...
import { environmentalAreaProtocolService } from '../../services/EnvironmentalAreaProtocolService.jsx';

export const useEnvironmentalAreaProtocolDelete = () => {
    const { showSuccessToast, showServerErrorsToast, toast } = usePRToast();
    const [loading, setLoading] = useState(false);

    const deleteEnvironmentalAreaProtocol = async (id: string) => {
        let confirmed = false
        try {
            ...
            showSuccessToast({ title: 'Protocolo eliminado', message: 'El protocolo se ha eliminado correctamente' })
            return confirmed
        } catch (err) {
