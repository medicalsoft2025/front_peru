...
export const TableMenuActions: React.FC<{ rowData: BoletaModel }> = ({ rowData }) => {
  const menu = useRef<Menu>(null);

  // 📌 Aquí usamos el hook
  const { descargarPdfBoleta, descargarXmlBoleta, descargarCdrBoleta } = useBoletas();
...
const handleDownloadPdf = async () => {
  ...
  const handleDownloadCdr = async () => {
    try {
      ...
      const blob = new Blob([response], { type: "application/zip" }); // si el CDR viene comprimido
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
  ...
  const menuItems = [
    ...
    {
      label: "Descargar XML",
      icon: <i className="fas fa-file-code me-2"></i>,
      command: handleDownloadXml,
    },
    {
      label: "Descargar CDR",
      icon: <i className="fas fa-file-archive me-2"></i>,
