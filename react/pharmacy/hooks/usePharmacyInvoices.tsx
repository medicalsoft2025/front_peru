...
export const usePharmacyInvoices = () => {
    ...
    const fetchInvoices = async (params: any) => {
        ...
        try {
            ...
            const response = await invoiceService.filterInvoices(
                cleanJsonObject(filterParams)
            );

            setInvoices(response.data.map((invoice: any) => handleToMappedInvoice(invoice)));
    ...
    const handleToMappedInvoice = (response: any): pharmacyInvoicesI => {
        return {
            id: response.id,
    ...
    const formatDate = (dateString: string, withTime: boolean = false): string => {
