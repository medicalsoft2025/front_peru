...
export const useSystemConfiguration = ({ steps, initialStep = 0 }) => {
  ...
  const { isLoading, currentConfig, saveProgress, error, loadProgress } =
    useConfigurationProgress({
      onProgressLoaded: (config) => {
        ...
        if (
          configData &&
          configData.step_index !== undefined &&
          configData.step_index !== null
        ) {
          ...
          if (savedStepIndex >= 0 && savedStepIndex < steps.length) {
            ...
            console.warn(
              "❌ Índice de paso guardado inválido:",
              savedStepIndex
            );
  ...
  useEffect(() => {
    ...
    if (
      lastSavedStepRef.current.stepId !== currentStepInfo.stepId ||
      lastSavedStepRef.current.stepIndex !== currentStepInfo.stepIndex
    ) {
      saveProgress(currentStep.id, activeIndex)
        .then(() => {
