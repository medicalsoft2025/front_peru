...
export const useCompanyMutation = () => {
  ...
  const guardarInformacionGeneral = async (
    ...
    try {
      ...
      if (marcaAguaFile) {
        try {
          const uploadedWatermark = await uploadFile(
            marcaAguaFile,
            finalCompanyId,
            "watermark"
          );
      ...
      if (logoUrl) {
        infoGeneralCompleta.logo = `App\\Models\\logoFile/${logoUrl
          .split("/")
          .pop()}`;
      }
      if (watermarkUrl) {
        infoGeneralCompleta.watermark = `App\\Models\\waterMark/${watermarkUrl
          .split("/")
          .pop()}`;
      }
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : "Error al guardar la información general";
