...
async function guardarArchivo(formData: FormData, flag?: boolean) { /*...*/ }
async function getUrlImage(path: string, flag?: boolean) { /*...*/ }
...
export const useAsignarConsentimiento = () => {
  ...
  const [documentToView, setDocumentToView] = useState<DocumentoConsentimiento | null>(null);
  ...
  const handleDeleteDocument = async (id: string) => {
    ...
    try {
      ...
      toast.current?.show({ severity: "success", summary: "Eliminado", detail: "Documento eliminado correctamente", life: 3000 });
    } catch {
  ...
  const handleViewDocument = (id: string) => {
    const doc = documents.find(d => d.id === id);
    if (!doc) {
      toast.current?.show({ severity: "warn", summary: "No encontrado", detail: "No se encontró el documento", life: 3000 });
      return;
    }
  ...
  const handleSubmitDocument = async (formData: any, template: ConsentimientoData) => {
    try {
      ...
      if (currentDocument) {
        ...
        toast.current?.show({ severity: "info", summary: "Actualizado", detail: "Documento actualizado con éxito", life: 3000 });
      } else {
        await createTemplate(payload);
        toast.current?.show({ severity: "success", summary: "Creado", detail: "Documento creado con éxito", life: 3000 });
      }

      setShowDocumentFormModal(false);
      setCurrentDocument(null);
    } catch (error) {
      toast.current?.show({ severity: "error", summary: "Error", detail: "No se pudo guardar el documento", life: 3000 });
    }
  };
      ...
      try {
        ...
        guardarArchivo(formData, true)
          .then((response: any) => {
            ...
            //   id: response.file.id,
            // });
            toast.current?.show({ severity: "success", summary: "Éxito", detail: "Documento generado y subido correctamente.", life: 3000 });
          })
          .catch(err => {
            toast.current?.show({ severity: "error", summary: "Error", detail: "No se pudo completar la operación.", life: 3000 });
            reject(err);
          });
      } catch (err) {
        toast.current?.show({ severity: "error", summary: "Error", detail: "No se pudo completar la operación.", life: 3000 });
