...
export const useCompaniesCrud = () => {
    ...
    const fetchCompanies = async () => {
        try {
            ...
            toast.current?.show({ severity: 'error', summary: 'Error', detail: 'Error al cargar empresas' });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const deleteCompany = async (company: Company) => {
        try {
            ...
            toast.current?.show({ severity: 'error', summary: 'Error', detail: 'Error al eliminar empresa' });
        } finally {
            setIsLoading(false);
        }
    };
