...
export const useUserRoleUpdate = () => {
    ...
    const updateUserRole = async (id: string, data: UserRoleFormInputs) => {
        ...
        try {
            ...
            const activeCount = payload.menus.filter(
                (menu) => menu.is_active
            ).length;
            const inactiveCount = payload.menus.filter(
                (menu) => !menu.is_active
            ).length;
            ...
            console.log(
                `Ejemplos - Menú 1 (Home): ${data.menuIds.includes(1)}`
            );
            console.log(
                `Ejemplos - Menú 2 (Inicio): ${data.menuIds.includes(2)}`
            );
            console.log(
                `Ejemplos - Menú 23 (Reportes): ${data.menuIds.includes(23)}`
            );
            console.log(
                `Ejemplos - Menú 30 (Administración): ${data.menuIds.includes(
                    30
                )}`
            );
