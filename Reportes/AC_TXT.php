...
if($paciente == '1'){
    ...
    $queryU=mysqli_query($connMedical,"SELECT * FROM cliente where tipo_cliente in ('CC','RC','TI') $where ");
    ...
    $queryU=mysqli_query($connMedical,"SELECT * FROM cliente where tipo_cliente not in ('CC','RC','TI') $where");
    while($rowListaU=mysqli_fetch_array($queryU))
    {
        $ArregloCliente[] = $rowListaU['cliente_id'];
    }
