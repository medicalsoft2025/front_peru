...
import { rips, typeConsults, externalCauses, purposeConsultations } from "../../../services/commons.js"; // Components
  ...
  const fetchEvents = React.useCallback(async (fetchInfo, successCallback, failureCallback) => {
    try {
      ...
      const response = await appointmentService.filterAppointments({
        per_page: 50,
        // Increased from 10 to ensure we get enough events
        page: 1,
        search: "",
      ...
      const filteredAndMapped = appointments.filter(appointment => {
