...
export function useGeneralAIChat({
  username
}) {
  ...
  useEffect(() => {
    if (historyData) {
      ...
      const historyMessages = historyData.flatMap((item, index) => {
        const msgs = [];
  ...
  const sendQuestion = useCallback(async question => {
    ...
    try {
      ...
      if (result.data?.message?.response?.respuesta) answer = result.data.message.response.respuesta;else if (result.message?.response?.respuesta) answer = result.message.response.respuesta;else if (result.data?.respuesta) answer = result.data.respuesta;else if (result.data?.response) answer = result.data.response;else if (result.data?.content) answer = result.data.content;else if (typeof result.data === 'string') answer = result.data;
