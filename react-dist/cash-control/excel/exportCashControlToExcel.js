...
// Función para transformar los datos de cierres a un formato compatible con exportToExcel
const transformCierresToExcelData = cierres => {
  ...
  cierres.forEach(cierre => {
    cierre.details?.forEach(detalle => {
      ...
      totalesPorMetodo[detalle.payment_method_name].esperado += Number(detalle.total_expected) || 0;
      totalesPorMetodo[detalle.payment_method_name].recibido += Number(detalle.total_received) || 0;
      totalesPorMetodo[detalle.payment_method_name].diferencia += Number(detalle.remaining_amount) || 0;
    });
  });

  // Calculamos los totales generales
  const totalGeneral = {
    esperado: cierres.reduce((sum, c) => sum + (Number(c.total_expected) || 0), 0),
    recibido: cierres.reduce((sum, c) => sum + (Number(c.total_received) || 0), 0),
    diferencia: cierres.reduce((sum, c) => sum + (Number(c.remaining_amount) || 0), 0)
  };
  ...
  const rows = cierres.flatMap(cierre => {
    ...
    const detailRows = cierre.details?.map(detalle => ({
      Control: "",
      Fecha: "",
      "Total Recibido": "",
      Sobrante: "",
...
export const exportToExcel = async options => {
  try {
    ...
    window.saveAs(blob, `${fileName}_${new Date().getTime()}${EXCEL_EXTENSION}`);
  } catch (error) {
...
export const exportCierresCajaToExcel = async options => {
  const {
    cierres,
  ...
  try {
    ...
    alert("Error al exportar los cierres de caja a Excel. Por favor recarga la página.");
    throw error;
  }
};