...
export const useGoogleCalendarPayload = () => {
   ...
   */
  const organizeEventPayload = (patientId, patientName, appointmentDate, appointmentTime, durationMinutes = 30, reason = 'Consulta médica') => {
    ...
    try {
      ...
      const formattedStartTime = appointmentTime.includes(':') ? appointmentTime.length === 5 ? appointmentTime + ':00' : appointmentTime : appointmentTime + ':00:00';
   ...
   */
  const organizeConfigPayload = (config, userEmail, userId) => {
    return {
   ...
   */
  const organizeMultipleEventsPayload = appointments => {
    return appointments.map(appointment => organizeEventPayload(appointment.patientId, appointment.patientName, appointment.appointmentDate, appointment.appointmentTime, appointment.durationMinutes, appointment.reason)).filter(payload => payload !== null);
  };
   ...
   */
  const isValidEventPayload = payload => {
    return !!payload.user_id && !!payload.nombre && !!payload.fecha && !!payload.hora && !!payload.hora_final && !!payload.motivo;
  };

  /**
   * Valida si un payload de configuración es válido
   */
  const isValidConfigPayload = payload => {
    return !!payload.user_email && !!payload.calendar_id && !!payload.timezone && !!payload.user_id;
  };
  return {
