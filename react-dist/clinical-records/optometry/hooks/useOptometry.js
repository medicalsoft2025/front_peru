...
import OptometryService from "../../../../services/api/classes/optometryService.js";
export const useOptometry = () => {
  ...
  const getRecipeInvoiceStatus = async recipeId => {
    try {
      ...
      console.error(`Error fetching invoice status for recipe ${recipeId}:`, err);
      throw err;
    }
  };
  ...
  const createOptometryRecipe = async (data, patientId) => {
    try {
      const service = new OptometryService();
      const response = await service.createOptometryRecipe(data, patientId);
