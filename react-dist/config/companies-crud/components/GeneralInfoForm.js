...
import { useCompanyMutation } from "../../company-configuration/hooks/useCompanyMutation.js";
...
const GeneralInfoForm = ({
  ...
  useEffect(() => {
    if (logoValue && !newLogoFile) loadImagePreview(logoValue, "logo");else if (!logoValue && !newLogoFile) setLogoPreview(null);
  }, [logoValue, newLogoFile]);
  useEffect(() => {
    if (watermarkValue && !newWatermarkFile) loadImagePreview(watermarkValue, "watermark");else if (!watermarkValue && !newWatermarkFile) setWatermarkPreview(null);
  ...
  const loadImagePreview = async (imagePath, type) => {
    try {
      ...
      if (typeof getUrlImage === "function") {
        ...
        if (type === "logo") setLogoPreview(imageUrl);else setWatermarkPreview(imageUrl);
      } else {
  ...
  const handleLogoChange = event => {
    ...
    if (file) {
      ...
      setNewLogoFile(null);
      if (logoValue) loadImagePreview(logoValue, "logo");else setLogoPreview(null);
    }
  };
  const handleWatermarkChange = event => {
    ...
    if (file) {
      ...
      setNewWatermarkFile(null);
      if (watermarkValue) loadImagePreview(watermarkValue, "watermark");else setWatermarkPreview(null);
    }
  };
  const removeLogo = () => {
    setNewLogoFile(null);
  ...
  const onSubmit = async data => {
    try {
      const response = await guardarInformacionGeneral(data, newLogoFile || undefined, newWatermarkFile || undefined, company?.id);
